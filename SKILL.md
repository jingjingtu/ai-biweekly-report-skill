...
- 主色表达内容类别：绿色为主线，紫色为工程化/生成能力，青色为验证/连接，粉色为差异、分支或需要注意的局部语义；B 端汇报不使用红色。完成状态使用独立状态标签，不依赖颜色。
...
- B 端章节大标题默认只使用白色主体与主绿关键词，不用红色作为大面积标题强调。问题和失败通过小号眉题、状态标签、局部描边或短注释表达。
- B 端长卷中的路径条、流程条、组合方式条和状态芯片必须保留对称内边距；首尾内容不能贴边，流程箭头必须位于连接间隙的视觉中心，状态文案在芯片内水平、垂直居中。
- B 端长卷不能只由透明黑底和线条框组成：按章节关系加入低对比深色渐变或层级表面，并选用流程轨道、阶段时间轴、分支连接、汇聚节点或证据载体等可读的辅助逻辑元素。渐变和装饰必须服务分组、方向或主次，不得制造无意义光效。
