...
仓库内包含经过审查的 [视觉样例规则](references/sample-standard.md) 和用户明确授权公开保存的 [12 张原始视觉参考](references/visual-reference-index.md)。其他团队样例只提供视觉构图，用户 B 端历史汇报提供内容逻辑与证据组织；两类资产都不会自动把原样例中的品牌、项目事实或数据带入新报告。
